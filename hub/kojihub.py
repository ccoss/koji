...
def list_pkgs(buildID=None, buildrootID=None, imageID=None, componentBuildrootID=None, hostID=None, arches=None, queryOpts=None):
    """List PKGS.  If buildID, imageID and/or buildrootID are specified,
    restrict the list of PKGs to only those PKGs that are part of that
    build, or were built in that buildroot.  If componentBuildrootID is specified,
    restrict the list to only those PKGs that will get pulled into that buildroot
    when it is used to build another package.  A list of maps is returned, each map
    containing the following keys:
    ...
    If no build has the given ID, or the build generated no PKGs,
    an empty list is returned."""
    fields = [('pkginfo.id', 'id'), ('pkginfo.name', 'name'), ('pkginfo.version', 'version'),
              ...
              ('pkginfo.build_id', 'build_id'), ('pkginfo.buildroot_id', 'buildroot_id'),
              ('pkginfo.external_repo_id', 'external_repo_id'),
              ('external_repo.name', 'external_repo_name'),
             ]
    joins = ['external_repo ON pkginfo.external_repo_id = external_repo.id']
    clauses = []

    if buildID != None:
        clauses.append('pkginfo.build_id = %(buildID)i')
    if buildrootID != None:
        clauses.append('pkginfo.buildroot_id = %(buildrootID)i')
    if componentBuildrootID != None:
        fields.append(('buildroot_listing.buildroot_id as component_buildroot_id',
                       'component_buildroot_id'))
        fields.append(('buildroot_listing.is_update', 'is_update'))
        joins.append('buildroot_listing ON pkginfo.id = buildroot_listing.pkg_id')
        clauses.append('buildroot_listing.buildroot_id = %(componentBuildrootID)i')

    # image specific constraints
    if imageID != None:
       clauses.append('imageinfo_listing.image_id = %(imageID)i')
       joins.append('imageinfo_listing ON pkginfo.id = imageinfo_listing.pkg_id')

    if hostID != None:
        joins.append('buildroot ON pkginfo.buildroot_id = buildroot.id')
        clauses.append('buildroot.host_id = %(hostID)i')
    if arches != None:
        if isinstance(arches, list) or isinstance(arches, tuple):
            clauses.append('pkginfo.arch IN %(arches)s')
        elif isinstance(arches, str):
            clauses.append('pkginfo.arch = %(arches)s')
        else:
            raise koji.GenericError, 'invalid type for "arches" parameter: %s' % type(arches)
...
class RootExports(object):
    ...
    def getChangelogEntries(self, buildID=None, taskID=None, filepath=None, author=None, before=None, after=None, queryOpts=None):
        """Get changelog entries for the build with the given ID,
           or for the pkg generated by the given task at the given path
        ...
        if buildID:
            ...
            if not build_info:
                return _applyQueryOpts([], queryOpts)
            spkgs = self.listPKGs(buildID=build_info['id'], arches='src')
            if not spkgs:
                return _applyQueryOpts([], queryOpts)
            spkg_info = spkgs[0]
            spkg_info['pm_name'] = build_info['pm_name']
            spkg_path = os.path.join(koji.pathinfo.build(build_info), koji.pathinfo.pkg(spkg_info))
        elif taskID:
            if not filepath:
                raise koji.GenericError, 'filepath must be spcified with taskID'
            if filepath.startswith('/') or '../' in filepath:
                raise koji.GenericError, 'invalid filepath: %s' % filepath
            spkg_path = os.path.join(koji.pathinfo.work(),
                                     koji.pathinfo.taskrelpath(taskID),
                                     filepath)
        else:
            raise koji.GenericError, 'either buildID or taskID and filepath must be specified'

        if not os.path.exists(spkg_path):
            return _applyQueryOpts([], queryOpts)
        ...
        fields = koji.createPkgInfo(spkg_path).getInfo(['changelogtime', 'changelogname', 'changelogtext'])
    ...
    listPKGs = staticmethod(list_pkgs)
    ...
    def listBuildPKGs(self,build):
        ...
        if not isinstance(build, int):
            #lookup build id
            build = self.findBuildID(build)
        return self.listPKGs(buildID=build)
